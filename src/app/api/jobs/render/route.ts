...
async function validateR2File(r2Key: string): Promise<{ valid: boolean; error?: string; size?: number }> {
  try {
...
async function callTriggerDev(jobId: string, r2Key: string, targetLang: string): Promise<{ runId: string; error?: string }> {
  try {
    const triggerUrl = `${process.env.TRIGGER_API_URL || 'https://api.trigger.dev'}/api/v1/jobs/render`;
...
function validateRequestBody(body: RenderJobRequest): { valid: boolean; error?: string } {
  if (!body.r2Key || !body.targetLang) {
    return { valid: false, error: 'Missing required fields: r2Key and targetLang' };
  }

  if (body.targetLang !== 'vi') {
    return { valid: false, error: 'Currently only Vietnamese (vi) target language is supported' };
  }

  return { valid: true };
}

async function validateJobRequest(body: RenderJobRequest, clientIP: string): Promise<NextResponse | null> {
...
async function processRenderJob(request: NextRequest): Promise<NextResponse> {
  ...
  // Call Trigger.dev
  const triggerResult = await callTriggerDev(jobId, body.r2Key, body.targetLang);
